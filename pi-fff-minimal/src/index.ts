...
export default function fffExtension(pi: ExtensionAPI) {
  ...
  let currentMode: FffMode = "override";
  ...
  function resolveStartupConfig(): void {
    setMode(
      getConfigValue("fff-mode", "PI_FFF_MODE", config.mode, "override", parseMode),
    );
  ...
  const grepSchema = Type.Object({
    ...
    exclude: Type.Optional(
      Type.Union([Type.String(), Type.Array(Type.String())], {
        description: "Optional path or glob exclusions.",
      }),
    ),
    caseSensitive: Type.Optional(
      Type.Boolean({
        description: "Force exact case; otherwise smart-case.",
      }),
    ),
  ...
  queueTool(() => toolNames.grep, {
    description: "Indexed content search with smart-case, regex detection, frecency ranking, and git awareness.",
    promptSnippet: "Indexed content search",
  ...
  const findSchema = Type.Object({
    pattern: Type.String({
      description: "Fuzzy repo-relative path query. Use for filenames and paths, not file contents.",
    }),
    path: Type.Optional(
      Type.String({
        description: "Optional path or glob filter.",
  ...
  queueTool(() => toolNames.find, {
    description: "Indexed fuzzy path search with frecency ranking and git awareness.",
    promptSnippet: "Indexed path search",
  ...
  if (enableMultiGrep) {
    const multiGrepSchema = Type.Object({
      patterns: Type.Array(Type.String(), {
        description: "Literal patterns matched with OR logic.",
      }),
      constraints: Type.Optional(
        Type.String({ description: "Optional file filter." }),
      ),
      context: Type.Optional(
        Type.Number({
          description: "Context lines around matches.",
    ...
    queueTool(() => toolNames.multiGrep, {
      description: "Indexed OR search for multiple literal patterns.",
      promptSnippet: "Multi-pattern content search",
